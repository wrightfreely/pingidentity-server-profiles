...
if test ! -z "${ARTIFACT_S3_URL}"; then
  ...
  aws s3 ls "${ARTIFACT_S3_URL}" > ${OUT_DIR}/list.txt
    aws s3 cp "${ARTIFACT_S3_URL}/${ARTIFACT_NAME}/${ARTIFACT_VERSION}/deploy" "${OUT_DIR}/instance/server/default/deploy" > ${OUT_DIR}/error1.txt
