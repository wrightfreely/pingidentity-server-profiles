...
if [[ ! -z "${OPERATIONAL_MODE}" && "${OPERATIONAL_MODE}" = "CLUSTERED_ENGINE" ]]; then
    ...
    OUT=$( make_api_request https://${K8S_STATEFUL_SET_SERVICE_NAME_PA_INTERNAL}:9000/pa-admin-api/v3/httpsListeners )
    ...
    OUT=$( make_api_request https://${K8S_STATEFUL_SET_SERVICE_NAME_PA_INTERNAL}:9000/pa-admin-api/v3/keyPairs  )
    ...
    OUT=$( make_api_request https://${K8S_STATEFUL_SET_SERVICE_NAME_PA_INTERNAL}:9000/pa-admin-api/v3/engines/certificates )
    ...
    OUT=$( make_api_request https://${K8S_STATEFUL_SET_SERVICE_NAME_PA_INTERNAL}:9000/pa-admin-api/v3/engines )
    ENGINE_ID=$( jq -n "$OUT" | \
    ...
    if test -z "${ENGINE_ID}" || test "${ENGINE_ID}" = null ; then
        ...
        }" https://${K8S_STATEFUL_SET_SERVICE_NAME_PA_INTERNAL}:9000/pa-admin-api/v3/engines )
    ...
    make_api_request -X POST https://${K8S_STATEFUL_SET_SERVICE_NAME_PA_INTERNAL}:9000/pa-admin-api/v3/engines/${ENGINE_ID}/config \
    -o engine-config.zip
